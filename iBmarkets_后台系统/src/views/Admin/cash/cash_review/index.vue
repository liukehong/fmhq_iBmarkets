<template>
  <transition name="fade">
    <div class="cash_review" style="overflow: hidden;">
      <!-- 出金审核 -->
      <div class="item_title">{{ $t('title.cashReview') }}</div>
      <div class="item_box">
        <div class="item_table_box">
          ...
          >
            ...
            <el-table-column :label="params.spare1 == 1?$t('table.num'):$t('table.money1')">
              <template
                slot-scope="scope"
              >{{ !!scope.row.withdrawalApplyMoney?FNMONEY(scope.row.withdrawalApplyMoney):'--' }}</template>
            </el-table-column>
            <!-- 配套金额 -->
            <el-table-column :label="params.spare1 == 1?$t('table.num'):$t('table.money2')">
              <template
                slot-scope="scope"
              >{{ scope.row.withdrawalApplyDeductionMoney|FORMATTED_NUMBER }}</template>
            </el-table-column>
            <!-- 提现类型 -->
  ...
  methods: {
    ...
    fnExcelData() {
      ...
      require.ensure([], () => {
        ...
        const filterVal = [
          ...
          "withdrawalApplyDeductionMoney",
          "new_receiptType",
          "receiptAddress"
        ];
        const list = vm.fileterData(); // 数据过滤
        const data = vm.formatJson(filterVal, list);
        export_json_to_excel(tHeader, data, "出金审核记录");
      });
    },
    ...
    fnExcel() {
      ...
      require.ensure([], () => {
        ...
        export_json_to_excel(tHeader, data, "出金列表excel");
      });
    },
    formatJson(filterVal, jsonData) {
      return jsonData.map(v => filterVal.map(j => v[j]));
          ...
          .then(({ value }) => {
            vm.$api
              .SYSTEM_WITHDRAWALREVIEW({
                ...
                status: status,
                details: value,
                type: data.withdrawalApplyType == 0 ? 2 : 1
              })
              .then(res => {
                if (res.code == 0) {
                  vm.fnOpenMessageBox(vm.$t("alert.success"), "success");
