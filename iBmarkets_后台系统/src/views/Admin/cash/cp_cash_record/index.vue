<template>
  <transition name="fade">
    <div class="cash_record" style="overflow: hidden;">
      <!-- 出金记录 -->
      <div class="item_title">{{ $t('title.cpCashRecord') }}</div>
      <div class="item_box">
  ...
  methods: {
    ...
    fnExcelData() {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ["日期","用户名","金额","手续费",'币种',"钱包地址","状态","原因"];
        const filterVal = ["withdrawalApplyCreateTime","userAccount", "withdrawalApplyMoney",'new_money','new_receiptType','receiptAddress','new_withdrawalApplyStatus',"withdrawalApplyDetails"];
        const list = vm.fileterData(); // 数据过滤
        const data = vm.formatJson(filterVal, list);
        export_json_to_excel(tHeader, data, "CP出金记录");
      });
    },
