<template>
  <div class="item_table_box">
    ...
    >
      <!-- 团队等级ID -->
      <el-table-column :label="$t('admin_allocation.text9')">
        <template slot-scope="scope">{{ scope.row.gradeId }}</template>
      </el-table-column>
      <!-- 层次 -->
      <el-table-column :label="$t('admin_allocation.text7')">
        <template slot-scope="scope">{{ scope.row.gradeLevel }}</template>
      </el-table-column>
      <!-- 收益 -->
      <el-table-column :label="$t('admin_allocation.text3')">
        <template slot-scope="scope">{{ scope.row.gradeProfit }}</template>
      </el-table-column>
      <!-- 操作 -->
      <el-table-column width="250" :label="$t('admin_allocation.text4')">
        <template slot-scope="scope">
          <!-- 修改 -->
          <el-button :disabled="!!!btnFlag?true:false" @click="fnSeeInfo(scope.row,'edit2')" type="primary" size="mini">{{ $t('admin_allocation.text5') }}</el-button>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    fnGetData() {
      ...
      vm.$api.SYSTEM_GETGRADEPROFITMAPPER(vm.params).then(res => {
        if (res.code == 0) {
