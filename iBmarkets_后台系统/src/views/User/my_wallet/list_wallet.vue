        ...
        <div class="item_table_box">
          ...
          >
            ...
            <el-table-column :label="$t('table.money')">
              <template slot-scope="scope">{{ scope.row.walletLogMoney||0 }}</template>
            </el-table-column>
            <!-- 操作后金额 -->
            <el-table-column :label="$t('table.oldMoney')">
              <template slot-scope="scope">{{ scope.row.walletLogMoneyRear||0 }}</template>
            </el-table-column>
            <!-- 金额单位 -->
            <!-- <el-table-column :label="$t('table.moneyUnit')">
              <template slot-scope="scope">{{ scope.row.walletLogMoneyUnit == 0?$t('table.usd'):$t('table.rmb') }}</template>
            </el-table-column> -->
            <!-- 钱包类型 -->
            <el-table-column :label="$t('table.walletType')">
              <template slot-scope="scope">{{ scope.row.walletType == 1?'ap':'cp' }}</template>
            </el-table-column>
            <!-- 出/入金 -->
